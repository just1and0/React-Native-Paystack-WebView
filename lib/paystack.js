...
const Paystack = ({ modal = true, style = {}, paystackKey, billingEmail, phone, lastName, firstName, amount = '0.00', currency = 'NGN', channels = ['card'], refNumber, billingName, subaccount, handleWebViewMessage, onCancel, autoStart = false, onSuccess, activityIndicatorColor = 'green', }, ref) => {
    ...
    return modal === true ? (<react_native_1.Modal style={{ flex: 1 }} visible={showModal} animationType="slide" transparent={false}>
      <react_native_1.SafeAreaView style={{ flex: 1 }}>
        ...
        (
        <react_native_1.View style={[{ flex: 1 }, style]} visible={showModal} animationType="slide" transparent={false}>
            <react_native_1.SafeAreaView style={{ flex: 1 }}>
        <react_native_webview_1.WebView style={[{ flex: 1 }]} source={{ html: Paystackcontent }} onMessage={(e) => {
            messageReceived(e.nativeEvent?.data);
        }} onLoadStart={() => setisLoading(true)} onLoadEnd={() => setisLoading(false)} onNavigationStateChange={onNavigationStateChange} ref={webView} cacheEnabled={false} cacheMode={'LOAD_NO_CACHE'}/>
